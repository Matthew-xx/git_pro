...
func(this *UserController) HandleReg(){
	...
	res := reg.FindString(email)  //用正则匹配邮箱格式，匹配不上返回空
	if res == "" {
	...
	//发送邮件
	emailConfig := `{"username":"1659530762@qq.com","password":"wfjbvauikxvychha","host":"smtp.qq.com","port":587}`
	emailConn := utils.NewEMail(emailConfig)
	emailConn.From = "1659530762@qq.com"
	emailConn.To = []string{email}
	emailConn.Subject = "天天生鲜用户注册"
	//注意这里我们发送给用户的是激活请求地址
	emailConn.Text = "192.168.0.5:8080/active?id="+strconv.Itoa(user.Id)
...
func(this *UserController) HandleLogin(){
	...
	err := o.Read(&user,"Name") //根据Name字段查询，若无误则将所有字段赋值到user，有错误返回到err
	if err != nil{
