...
func (tx *Transaction) HashTransaction()  {
	...
	//拼接的时候添加时间戳，避免奖励的coinbase与创世交易一致
...
func (tx *Transaction) Sign(privKey ecdsa.PrivateKey,prevTXs map[string]Transaction)  {
	...
	for inID,vin := range txCopy.Vins{
		...
		signature := append(r.Bytes(),s.Bytes()...) //拼接起来做签名数组
...
func (tx *Transaction) TrimmedCopy() Transaction {
	...
	for _,vin := range tx.Vins{
		inputs = append(inputs,&TXInput{vin.TxHash,vin.Vout,vin.Signature,vin.PublicKey})
	}
...
func (tx *Transaction) Verify(prevTXs map[string]Transaction) bool {
	...
	for _,vin := range tx.Vins{
		if prevTXs[hex.EncodeToString(vin.TxHash)].TxHash == nil {
			log.Panic("ERROR:previous transation is not found")
		}
	}
