...
func (blockchain *Blockchain)MineNewBlock(from []string,to []string,amount []string) {
	...
	for index,address := range from{
		amount_value , _ := strconv.Atoi(amount[index])
		tx := NewSimpleTransaction(address,to[index],amount_value,blockchain,txs)
		txs = append(txs,tx)
	}
...
func (blockchain *Blockchain) UnUTXOs(address string,txs []*Transaction) []*UTXO {
	...
	for _,tx := range txs{
		...
		if tx.IsCoinbaseTransaction() == false { //除创世区块外的其他交易
			for _,in := range tx.Vins{
				...
				hashPubKey := publickeyHash[1:len(publickeyHash)-4] //获取公钥hash
				//是否能够解锁(用户姓名是否相等)
				if in.UnlockPubKey(hashPubKey) {
					//如果为真，则说明是所取的用户
					//存储，id做key,
	...
	for _,tx := range txs{
		...
		for index,out := range tx.Vouts{
			if out.UnlockScriptPubKeyWithAddress(address) { //out中对应的用户解锁才进行后面的步骤
							...
							for txHash,indexArray := range spentTxOutputs{
								...
								for _,i := range indexArray{
									if index == i && txHash == hex.EncodeToString(tx.TxHash) {
										//index是上一个输出的hash，说明这笔钱已被花费
										isSpentUTXO = true
										continue work
									}
								}
							}
							//for循环遍历后没有对应的输入 （if语句index == i
...
func (blockchain *Blockchain) FindSpentableUTXOs(from string,amount int,txs []*Transaction) (int64,map[string][]int) {
	...
	if value < int64(amount){
		fmt.Printf("%s 's fund not enough",from)
		os.Exit(1)
	}
...
func (blockchain *Blockchain) GetBalance(address string) int64 {
	...
	utxos := blockchain.UnUTXOs(address,[]*Transaction{}) //对于查询余额只需传空[]*Transaction{}
